...
extension GameScene {
	...
	func performAttack() {
		...
		if hero.currentHealth == 0 {
			gameOver(isPlayerVictory: false)
		} else if enemy.currentHealth == 0 {
			gameOver(isPlayerVictory: true)
		}
	}
...
extension GameScene {
	...
	private func setupGameAreas() {
		...
		let topArea = SKShapeNode(rect: CGRect(x: 0, y: size.height - topHeight,
	...
	private func setupHeroUI() {
		...
		let heroPosition = CGPoint(x: size.width * 0.33,
								   y: size.height * 0.3 + size.height * 0.5 / 2)
	...
	private func setupEnemyUI() {
		...
		let enemyPosition = CGPoint(x: size.width * 0.66,
									y: size.height * 0.3 + size.height * 0.5 / 2)
	...
	private func setupButtons() {
		...
		attackButton.position = CGPoint(x: size.width / 2,
										y: bottomHeight / 2 + 40)
		...
		roundLabel.position = CGPoint(x: size.width / 2,
									  y: size.height - topHeight / 2)
		addChild(roundLabel)

		endTurnButton = SKShapeNode(rectOf: buttonSize, cornerRadius: 10)
		endTurnButton.position = CGPoint(x: size.width / 2,
	...
	private func setupResultScreen() {
		// Container
		gameOverScreen.position = CGPoint(x: size.width / 2,
										  y: size.height / 2)
...
extension GameScene {
	func gameOver(isPlayerVictory: Bool) {
		...
		if let buttonLabel = actionButton.children.first as? SKLabelNode {
			buttonLabel.text = isPlayerVictory
				? "Enter New Room"
				: "Try Again"
		}
